...
export default function PageHeader({ onCreateClick, searchQuery, onSearchChange }: PageHeaderProps) {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
      <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'text.secondary', typography: 'body2' }}>
          ...
          <Typography variant="body2" sx={{ cursor: 'pointer', '&:hover': { color: 'primary.main' } }}>Advertisements</Typography>
          <Typography variant="body2">›</Typography>
          <Typography variant="body2" color="primary.main" sx={{ fontWeight: 500 }}>All Advertisements</Typography>
        </Box>
      </Box>

      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <TextField
          placeholder="Search by ad name, type, placement..."
          size="small"
          value={searchQuery || ''}
        ...
        <IconButton sx={{ bgcolor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 2 }}>
          <GridViewOutlinedIcon sx={{ color: '#64748b' }} />
        </IconButton>
